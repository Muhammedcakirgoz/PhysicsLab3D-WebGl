...
const controlMode = setupGUI(objects, cam);

function animate() {
  requestAnimationFrame(animate);
  
  // Kontrol moduna göre işlem yap
  if (controlMode.mode === 'camera') {
    camCtrl.update();
  } else {
    // Seçili nesneyi bul ve hareket ettir
    const selectedObject = objects.find(o => o.name === controlMode.selectedObject);
    if (selectedObject) {
      moveObject(selectedObject, keyState);
      // DEBUG: Konsola seçili nesne ve pozisyonunu yaz
      // console.log('Seçili nesne:', selectedObject.name, selectedObject.pos);
    }
  }
